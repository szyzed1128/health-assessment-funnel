  ...
  async findResultAccess(sessionId: string): Promise<ResultAccessRecord> {
    ...
    return {
      ...
      healthResult: session.healthResult === null ? null : {
        ...
        requestedTargetDate: session.healthResult.requestedTargetDate,
        targetDateSource: session.healthResult.targetDateSource,
        forecastTargetDate: session.healthResult.forecastTargetDate ?? session.healthResult.targetDate ?? new Date(0),
