...
| Mock payment activates the correct session subscription | `POST /api/pay` accepts `sessionId`, a unique `paymentEventId` and the fixed server-validated demo `paymentCode`; the server chooses payment success and membership dates. | Integration test reads subscription state after payment. |
...
| APIs validate all external input | `sessionId` and `paymentEventId` are UUIDs, and `paymentCode` is a non-empty string checked against the server-side demo code; a session must exist and have a persisted assessment result before it can be paid. | Invalid payload, wrong payment code, missing result, and unknown-session API tests. |
...
The PRD-compatible root path `POST /pay` reuses the same handler.

Request body:

```json
{ "sessionId": "UUID", "paymentEventId": "UUID", "paymentCode": "RQKJ-DEMO-2026" }
```

- The handler validates the fixed demo `paymentCode`; it accepts no price,
  duration, member flag, result data, or payment success flag from the client.
...
valid paymentCode + new paymentEventId -> create SUCCEEDED event + activate 30-day subscription
invalid paymentCode -> reject without creating an event or subscription
